// OPD LoggerX – v15 (web PWA + Capacitor-native export)
...
function buildSelectors() {
  ...
  // Disposition chips (one row via CSS)
...
function renderSummary() {
  ...
  const cont = document.getElementById("top-diags"); if (cont) { cont.innerHTML = ""; top.forEach(([name, c]) => { const div = document.createElement("div"); div.textContent = `${name}: ${c}`; cont.appendChild(div); }); }
}
...
/* ---------- Native-aware export helpers (Capacitor) ---------- */
...
async function saveAndShareNative(filename, mime, base64Data) {
  ...
  const Dir = FS?.Directory || { Documents: 'DOCUMENTS' };
  if (!FS || !Share) { console.error("Capacitor FS/Share missing"); return; }
  try {
    await FS.writeFile({ path: `OPD/${filename}`, data: base64Data, directory: Dir.Documents, recursive: true });
    const uriRes = await FS.getUri({ path: `OPD/${filename}`, directory: Dir.Documents });
    await Share.share({ title: `Export ${filename}`, text: `Exported ${filename}`, url: uriRes.uri, dialogTitle: "Share/export file" });
  } catch (e) {
...
async function downloadCSV(list) {
  ...
  if (isNative()) await saveAndShareNative(filename, "text/csv", toBase64(csv));
  else {
...
async function downloadXLS(list) {
  ...
  if (isNative()) await saveAndShareNative(filename, "application/vnd.ms-excel", toBase64(html));
  else {
